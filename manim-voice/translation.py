...
# The following function uses LOCALE and DOMAIN to set the language,
# and returns a gettext function that is used to insert translations.
_ = get_gettext()
...
class TranslationExample(VoiceoverScene):
    def construct(self):
        ...
        # This means that their translations will be looked up
        # in the .po files and used to replace the original strings
        with self.voiceover(
                text=_("This circle is drawn as I speak.")
        ) as tracker:
            self.play(
                Create(circle),
        ...
        with self.voiceover(
                text=_("Let's shift it to the left 2 units.")
        ) as tracker:
            self.play(
